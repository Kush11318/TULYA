...
export default function SearchBar({ onSearch, initialValue = '' }: SearchBarProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full">
      <div className="flex gap-2 rounded-full border border-sky-100 bg-white/90 p-1.5 shadow-sm ring-1 ring-slate-100 backdrop-blur-sm">
        <input
          type="text"
          value={productName}
          onChange={(e) => setProductName(e.target.value)}
          placeholder="Search for a product, e.g. “iPhone 15 Pro”"
          className="flex-1 rounded-full border-0 bg-transparent px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-0"
          disabled={isLoading}
        />
        <button
          type="submit"
