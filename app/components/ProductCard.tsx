  ...
  return (
    <div
      className={`
        ...
        dark:bg-slate-900/90 dark:border-slate-800 dark:hover:border-sky-500
        ${isSelected ? 'ring-1 ring-sky-400 ring-opacity-40' : ''}
      `}
    >
      {/* Checkbox for comparison */}
      <div className="absolute top-3 right-3 z-10" onClick={(e) => e.stopPropagation()}>
        <label className="flex items-center gap-2 cursor-pointer bg-white/80 rounded px-1 backdrop-blur-sm dark:bg-slate-800/80">
          <input
            type="checkbox"
            checked={isSelected}
          ...
          />
          <span className="text-xs text-gray-500 font-medium dark:text-gray-400">Compare</span>
        </label>
      </div>
      ...
      {/* Product Image */}
      <div className="relative h-28 w-28 shrink-0 overflow-hidden rounded-lg bg-gray-50 dark:bg-slate-800">
        {imageUrl && !imageError ? (
          <Image
            src={imageUrl}
            alt={product.title}
          ...
          <div className="w-full h-full flex items-center justify-center text-gray-400 text-sm dark:text-gray-500">
            No image
          </div>
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col justify-between space-y-1">
        {/* Title */}
        <div>
          <h3 className="line-clamp-2 text-sm font-medium leading-snug text-slate-900 pr-20 dark:text-slate-100">
            {product.title}
          </h3>
          <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
            <span className="rounded-full bg-slate-50 px-2 py-0.5 dark:bg-slate-800 dark:text-slate-300">{product.site}</span>
        ...
        <div className="mt-2 flex items-end justify-between">
          ...
          <div className="flex items-center gap-2">
            <a
              ...
              className="px-3 py-1.5 text-xs font-medium text-sky-700 bg-sky-50 rounded-full hover:bg-sky-100 transition-colors border border-sky-200 no-underline dark:bg-sky-900/30 dark:text-sky-300 dark:border-sky-800 dark:hover:bg-sky-900/50"
              onClick={(e) => {
                e.stopPropagation();
            ...
            <button
              onClick={copyLink}
              className="p-1.5 text-slate-400 hover:text-slate-700 transition-colors border border-slate-200 rounded-full dark:border-slate-700 dark:text-slate-500 dark:hover:text-slate-300"
              title="Copy Link"
            >
